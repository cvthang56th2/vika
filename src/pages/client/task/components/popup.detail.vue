<template>
  <AppPopup :value="value" @input="hide">
    <h4 v-if="!taskId">Popup Detail Task</h4>
    <h3>{{ taskId ? 'Edit' : 'Add' }} Task</h3>
...
<script>
import { saveItem, getEntities } from '@/services/task'

export default {
  name: 'PopupDetailTask',
  props: {
    value: {
      type: Boolean
    },
  ...
  methods: {
    ...
    async fetchData () {
      this.setLoading()
      try {
        const { data } = await getEntities({ taskId: this.taskId })
        if (data.taskObj) {
          this.formData = data.taskObj
        }
      } catch (error) {
