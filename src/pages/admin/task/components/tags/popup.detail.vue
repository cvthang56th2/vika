<template>
  <AppPopup :value="value" @input="hide">
    <h4>Popup Detail Task Tag</h4>
    <h3>{{ taskTagId ? 'Edit' : 'Add' }} Task Tag</h3>
...
<script>
import { saveItem, getEntities } from '@/services/task/tag'

export default {
  name: 'PopupDetailTaskTag',
  props: {
    value: {
      type: Boolean
    },
  ...
  methods: {
    ...
    async fetchData () {
      this.setLoading()
      try {
        const { data } = await getEntities({ taskTagId: this.taskTagId })
        if (data.taskTagObj) {
          this.formData = data.taskTagObj
        }
      } catch (error) {
